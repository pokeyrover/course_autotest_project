...
def test_guest_should_see_login_link_on_product_page(browser):                                      #checking present link to login page
    link = ProductPageLocators.PAGE_URL_WITHOUT_PROMO
    page = ProductPage(browser, link)
    page.open()
...
class TestUserAddToBasketFromProductPage:
    ...
    def setup(self, browser):                                                                               #setup will registrate new user before tests
        browser = browser
        ...
        page.open()
        page.go_to_login_page()
        page.registration_new_user(email, password)
        page.should_be_authorized_user()
        return browser
    
    def test_user_cant_see_success_message(self, browser):                                                   #will check not present a success message before add product
        browser = browser
        link = ProductPageLocators.PAGE_URL_WITHOUT_PROMO
        page = ProductPage(browser, link)
        page.open()
    ...
    @pytest.mark.need_review
    def test_user_can_add_product_to_basket(self, browser):                                            #will add product to basket and check success messages
        browser = browser
        link = ProductPageLocators.PAGE_URL_WITHOUT_PROMO
        page = ProductPage(browser, link)
        page.open()
